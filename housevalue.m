...
%训练集数据（预期输出）
R0 = [4925.00 	5937.00 	6716.71 	7288.24 	7197.00 	7032.00 	6875.00 	7504.00 	8733.00 	9866.60];
...
%建立BP神经网络
net = newff(pn, tn, 13, {'tansig', 'purelin'},'traingd'); 
...
%测试网络性能
testn = mapminmax('apply',test, inputStr);%归一化
sn = sim(net, testn); %测试
s = mapminmax('reverse', sn, outputStr); %反归一化

accuracy = 1- abs(s- testr(1))/testr(1);% 准确率

%年份
y = 2009:2019; Y = 2009:2022;
%指标数据，预期结果（训练集和测试集）
j = [p, test]; n = [R0, testr];
...
% 利用训练好的BP网络进行预测
gn = mapminmax('apply',g, inputStr); %归一化
sn = sim(net, gn); %预测
s = mapminmax('reverse',sn, outputStr);  %反归一化
